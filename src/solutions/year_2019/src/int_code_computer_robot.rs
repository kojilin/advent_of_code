...
pub struct IntCodeComputerRobot {
...
impl IntCodeComputerRobot {
    pub fn new(codes: &Vec<i64>) -> IntCodeComputerRobot {
        let mut program: BTreeMap<i64, i64> = BTreeMap::new();
        for (i, c) in codes.iter().enumerate() {
            program.insert(i as i64, *c);
        }

        IntCodeComputerRobot {
            program,
            offset: 0,
            halt: false,
            relative_base: 0,
    ...
    pub fn compute(&mut self, input: i64, expected_count: i32) -> Vec<i64> {
        let mut result: Vec<i64> = vec!();
        while result.len() < expected_count as usize && !self.is_halt() {
