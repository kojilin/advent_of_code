...
impl IntCodeComputer172 {
    pub fn new(codes: &Vec<i64>) -> IntCodeComputer172 {
    ...
    pub fn compute(&mut self, input: &mut Vec<i32>) -> Vec<i64> {
        ...
        while !self.is_halt() {
            ...
            match command % 100 {
                ...
                3 => {
                    let v1 = self.program.get(&(self.offset + 1)).unwrap_or(&(0 as i64)).clone();
                    self.read(command, v1, input.remove(0) as i64);
                }
                4 => {
                    let v1 = self.program.get(&(self.offset + 1)).unwrap_or(&(0 as i64)).clone();
